...
public class AccountAddressPageTest {
    ...
    @Rule
    public ActivityScenarioRule<AccountAddressPage> activityRule = new ActivityScenarioRule<>(new Intent(ApplicationProvider.getApplicationContext(), AccountAddressPage.class));

    @Test
    public void testVisibilityOfViews(){
        onView(withId(R.id.userAddressAddAddressButton)).check(matches(isDisplayed()));
    }

    @Test
    public void testAddressButtonClick(){
        onView(withId(R.id.userAddressAddAddressButton)).check(matches(isDisplayed())).perform(click());

        intended(hasComponent(AccountAddAddressPage.class.getName()));
    }
}
