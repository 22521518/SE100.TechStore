...
import com.example.electrohive.Activities.ReceiptPage;
...
public class ReceiptPageTest {
    ...
    private Intent createTestIntent(String paymentMethod) {
        Intent intent = new Intent(ApplicationProvider.getApplicationContext(), ReceiptPage.class);
        ...
        intent.putExtra("date",createDateNow());
    ...
    public void testVisibilityOfViewsWithCOD(){
        ...
        try (ActivityScenario<Activity> scenario = ActivityScenario.launch(intent)) {
            ...
            onView(withId(R.id.continue_shopping )).check(matches(isDisplayed()));
        }
    ...
    @Test
    public void testContinueShoppingClickZaloPay(){
        Intent intent = createTestIntent(PAYMENT_METHOD.ZALOPAY.name());

        try (ActivityScenario<Activity> scenario = ActivityScenario.launch(intent)) {
            onView(withId(R.id.continue_shopping )).check(matches(isDisplayed())).perform(click());
    ...
    @Test
    public void testContinueShoppingClickCOD(){
        Intent intent = createTestIntent(PAYMENT_METHOD.COD.name());
