...
export default function Tracking() {
  ...
  const fetchOrder = () => {
    ...
    setIsLoading(true)
    getOrder("userId", orderId).then((data) => {
      console.log(data);
      setOrder(data);
    });
  ...
  useEffect(() => {
    orderId ? fetchOrder() : null;
  }, []);

  const renderPaymentMethod = (method) => {
    switch (method) {
      case "COD":
        return <div className="flex flex-row gap-2 items-center font-bold">COD <FontAwesomeIcon icon={faMoneyBill}/></div>;
      case "CREDIT_CARD":
        return <div className="flex flex-row gap-2 items-center font-bold">Credit Card <FontAwesomeIcon icon={faCreditCard}/></div>;
      case "ELECTRO_WALLET":
        return <div className="flex flex-row gap-2 items-center font-bold">Electro Wallet <FontAwesomeIcon icon={faWallet}/></div>;
    }
  };
      ...
      <div className="flex flex-col gap-10 w-full max-w-[800px]">
        <div className="relative  w-full flex flex-col gap-2">
          <ul className="flex flex-row items-center justify-between ">
            <li className="relative flex items-center justify-center rounded-full size-4 md:size-6 bg-on-background">
              {['PENDING','CANCELLED'].includes(order?.order_status)&&<span className="bg-background rounded-full size-2 md:size-3 absolute"></span>}
            </li>
            <li className="grow border-t-4 border-on-background"></li>
            <li className="relative flex items-center justify-center rounded-full size-4 md:size-6 bg-on-background">
              {order?.order_status==='CONFIRMED'&&<span className="bg-background rounded-full size-2 md:size-3 absolute"></span>}
            </li>
            <li className="grow border-t-4 border-on-background"></li>
            <li className="relative flex items-center justify-center rounded-full size-4 md:size-6 bg-on-background">
              {order?.order_status==='SHIPPED'&&<span className="bg-background rounded-full size-2 md:size-3 absolute"></span>}
            </li>
            <li className="grow border-t-4 border-on-background"></li>
            <li className="relative flex items-center justify-center rounded-full size-4 md:size-6 bg-on-background">
              {order?.order_status==='DELIVERED'&&<span className="bg-background rounded-full size-2 md:size-3 absolute"></span>}
            </li>
          </ul>
        ...
        <div className="panel-1">
          <div className="flex flex-col md:flex-row gap-2 items-starts justify-between">
            <div className="flex flex-row gap-2 grow items-center">
              ...
              <button
                ...
                onClick={fetchOrder}
              >
                {isLoading?<FontAwesomeIcon icon={faSpinner} className='animate-spin'/>:<FontAwesomeIcon icon={faSearch} />}
              </button>
            </div>
          ...
          <div className="flex flex-row gap-2 items-center justify-between">
            ...
            <h3 className="text-lg">{renderPaymentMethod(order?.payment_method)}</h3>
          </div>
        </div>
